...
const ProfileHeader = ({ username, role }: { username: string; role: string }) => {
  const style = ROLE_STYLE[role] ?? ROLE_STYLE.USER;
  const initial = username?.charAt(0).toUpperCase() || "?";
...
const UserSection = ({ navigate }: { navigate: ReturnType<typeof useNavigate> }) => (
  <OrdersActivityCard navigate={navigate} />
);

// ─── Seller ───────────────────────────────────────────────────────────

const SellerSection = ({ navigate }: { navigate: ReturnType<typeof useNavigate> }) => (
  <>
    <OrdersActivityCard navigate={navigate} />
    <Card>
      <SectionTitle label="My Store" />
...
const AdminSection = ({ navigate }: { navigate: ReturnType<typeof useNavigate> }) => (
  <>
    <Card className="border-purple-100 bg-purple-50">
      <div className="flex items-center gap-3">
        <ShieldCheck className="w-5 h-5 text-purple-400" />
        <div>
          <p className="text-sm font-bold text-purple-700">Admin Access</p>
          <p className="text-xs text-purple-400">Full platform control enabled</p>
        </div>
      </div>
    </Card>
    <Card>
...
const UserProfilePage = () => {
  ...
  return (
    <div className="min-h-screen bg-rose-50 py-10 px-4 sm:px-6 lg:px-8">
      <div className="max-w-lg mx-auto flex flex-col gap-4">
        <ProfileHeader username={user ?? "User"} role={role ?? "USER"} />
        {role === "USER"   && <UserSection   navigate={navigate} />}
        {role === "SELLER" && <SellerSection navigate={navigate} />}
        {role === "ADMIN"  && <AdminSection  navigate={navigate} />}
      </div>
    </div>
